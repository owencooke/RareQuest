...
export class ZebraCatcher extends Phaser.Scene {
  ...
  create() {
    ...
    this.player = new MyPlayer(this, sizes.width / 2, sizes.height - 50, "up").setScale(4);
    this.player.setImmovable(true);
    this.player.body.allowGravity = false;
    this.player.setCollideWorldBounds(true);

    // Adjusted to add initial downward velocity
    this.target = this.physics.add.image(0, 0, "zebra").
    ...
    this.textScore = this.add.text(10, 10, "Score: 0", { // Adjusted position to top left
      font: "32px Arial",
      fill: "#d55e27",
      fontStyle: "bold",
    });
    ...
    const homeButton = this.add.image(sizes.width - 50, 50, 'home').setScrollFactor(0)
      .setOrigin(0, 0.5)
      .setScale(0.2)
      .setInteractive();
  ...
  update() {
    ...
    if (this.target.y >= sizes.height - 50 && (this.target.x < this.player.x || this.target.x > this.player.x + this.player.width)) {
      this.gameOver(false);
    }
